...
int getItag(int qn) {
	array<int> qns = {30000, 20000, 10000, 400, 250, 150, 80};
	array<int> tis = {101,272,303,248,247,244,243};
	int idx = tis.find(qn);
	if (idx >= 0) {
		return idx;
	}
...
string getLiveQn(int qn) {
	array<int>qns = {80, 150, 250, 400, 10000, 20000, 30000};
	array<string> qualities = {"流畅","高清","超清","蓝光","原画","4K","杜比"};
	int idx = qns.find(qn);
	if (idx>=0){
		return qualities[idx];
	}
...
string Live(string id, const string &in path, dictionary &MetaData, array<dictionary> &QualityList) {
	...
	if (Reader.parse(res, Root) && Root.isObject()) {
		...
		//ts stream
		string tsUrl = playurlInfo["playurl"]["stream"][1]["format"][0]["codec"][0]["url_info"][0]["host"].asString()+playurlInfo["playurl"]["stream"][1]["format"][0]["codec"][0]["base_url"].asString()+playurlInfo["playurl"]["stream"][1]["format"][0]["codec"][0]["url_info"][0]["extra"].asString();
		url = tsUrl;
		log("Tsurl",tsUrl);
		//fmp4 stream
		string fmp4Url = playurlInfo["playurl"]["stream"][1]["format"][1]["codec"][0]["url_info"][0]["host"].asString()+playurlInfo["playurl"]["stream"][1]["format"][1]["codec"][0]["base_url"].asString()+playurlInfo["playurl"]["stream"][1]["format"][1]["codec"][0]["url_info"][1]["extra"].asString();
		url = fmp4Url;
		log("Fmp4url",fmp4Url);
